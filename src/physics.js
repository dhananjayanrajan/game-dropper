...
export function checkCollisions(playArea, gameTime, shapesArray, mergingAnimationsArray, updateScore, currentMultiplier, comboManager, wakeUpShapeFn) {
    const {
    ...
    for (let i = 0; i < shapes.length; i++) {
        for (let j = i + 1; j < shapes.length; j++) {
            ...
            if (dist < minDist) {
                ...
                if (a.type === b.type && !a.isDitto && !b.isDitto && !a.isGold && !b.isGold) {
                    mergeShapes(a, b);
                    return;
                }
                const nx = dx / (dist || 1);
